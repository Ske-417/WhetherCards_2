  ...
  hours.forEach((h, idx) => {
    ...
    card.innerHTML = `
      ...
      <div class="card-face ${weatherCategory(h.code)}">
        ...
        <div class="card-icon" aria-hidden="true">${renderIcon(weatherCategory(h.code))}</div>
...
function renderIcon(category) {
  const src = ICON_ASSETS[category];
  const fallback = ICON_FALLBACK[category] || '？';
  return `<img class="weather-icon" src="${src}" alt="" onerror="this.replaceWith(document.createTextNode('${fallback}'))" />`;
}
